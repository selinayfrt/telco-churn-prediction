...
df = pd.read_csv("C:/Users/90545/telcocustomerchurn/Telco-Customer-Churn.csv")
df.head()
df.shape
df.info()
...
grid_search = GridSearchCV(catboost_model,
                           catboost_params,
                           cv=3,
                           scoring="roc_auc",
                           n_jobs=-1)
...
# En iyi model olarak CatBoost'u kullanıyoruz (tuned model)
...
print("TRAIN AUC:", cv_results["train_roc_auc"].mean())
print("VALIDATION AUC:", cv_results["test_roc_auc"].mean())

print("TRAIN F1:", cv_results["train_f1"].mean())
print("VALIDATION F1:", cv_results["test_f1"].mean())
##################################
...
plt.figure(figsize=(6,4))
plt.plot(fpr, tpr, label="CatBoost (AUC = %.3f)" % roc_auc_score(y_test, y_prob))
plt.plot([0,1],[0,1],'r--')
